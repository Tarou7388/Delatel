...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 29-01-2025 a las 13:06:03
...
-- Procedimientos
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `ObtenerHistorialSoporte` (IN `docCliente` VARCHAR(20))   BEGIN
    SELECT 
        s.id_soporte,
        s.id_contrato,
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `spu_actualizar_linea` (IN `p_id_caja` INT, IN `p_coordenadas` JSON, IN `p_id_user_create` INT)   BEGIN
  IF (p_id_caja = -1) THEN
    UPDATE tb_lineas
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `spu_antenas_listar` ()   BEGIN
  SELECT
        id_antena,
        id_distrito, 
        nombre, 
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `spu_antenas_registrar` (IN `p_id_distrito` INT, IN `p_nombre` VARCHAR(60), IN `p_descripcion` VARCHAR(100), IN `p_coordenadas` VARCHAR(50), IN `p_direccion` VARCHAR(200), IN `p_iduser` INT)   BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `spu_antena_inhabilitar` (IN `p_id_antena` INT, IN `p_iduser` INT)   BEGIN
  UPDATE tb_antenas 
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `spu_buscar_cajas_sector_idCaja` (IN `p_id_caja` INT)   BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `spu_caja_eliminar` (IN `p_id_caja` INT, IN `p_id_user` INT)   BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `spu_caja_uso` (IN `p_id_caja` INT)   BEGIN
  SELECT 
    CASE 
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `spu_contratos_actualizar` (IN `p_id_contrato` INT, IN `p_id_paquete` INT, IN `p_direccion_servicio` VARCHAR(200), IN `p_referencia` VARCHAR(200), IN `p_nota` TEXT, IN `p_iduser_update` INT)   BEGIN
    UPDATE tb_contratos
    SET
        id_paquete = p_id_paquete,
        direccion_servicio = p_direccion_servicio,
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `spu_contratos_registrar` (IN `p_id_cliente` INT, IN `p_id_paquete` INT, IN `p_id_sector` INT, IN `p_direccion_servicio` VARCHAR(200), IN `p_referencia` VARCHAR(200), IN `p_coordenada` VARCHAR(50), IN `p_fecha_inicio` DATE, IN `p_fecha_registro` DATE, IN `p_nota` TEXT, IN `p_ficha_instalacion` JSON, IN `p_iduser_create` INT)   BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `spu_mufa_eliminar` (IN `p_id_mufa` INT, IN `p_id_user` INT)   BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `spu_mufa_uso` (IN `p_id_mufa` INT)   BEGIN
  SELECT
    CASE
      WHEN COUNT(*) > 0 THEN TRUE
      ELSE FALSE
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `spu_productos_buscar_barraRepetidor` (IN `p_codigo_barra` VARCHAR(120))   BEGIN
    SELECT
        p.id_producto,
        p.modelo,
        p.precio_actual,
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `spu_productos_buscar_barraSintonizador` (IN `p_codigo_barra` VARCHAR(120))   BEGIN
    SELECT
        p.id_producto,
        p.modelo,
        p.precio_actual,
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `spu_sector_desactivar` (IN `p_id_sector` INT, IN `p_id_user` INT)   BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `spu_soporte_actualizar` (IN `p_id_soporte` INT, IN `p_id_tecnico` INT, IN `p_fecha_hora_asistencia` DATETIME, IN `p_soporte` JSON, IN `p_iduser_update` INT, IN `p_procedimiento_S` TEXT)   BEGIN
    UPDATE tb_soporte
    SET
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `spu_soporte_pdf` (IN `p_id_soporte` INT)   BEGIN
    SELECT 
        s.id_soporte,
        s.id_contrato,
        pa.paquete,
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `spu_usuario_actualizar` (IN `p_nombre_user` VARCHAR(100), IN `p_iduser_update` INT, IN `p_id_usuario` INT)   BEGIN
	UPDATE tb_usuarios
...
(1, 'Almacen 1', 'Av 123', '-13.415960344185644, -76.13418174072265', NULL, NULL, '2025-01-29 06:57:08', NULL, 1, NULL),
(2, 'Almacen 2', 'Av 123456', '-13.427602649212902, -76.13979326342246', NULL, NULL, '2025-01-29 06:57:08', NULL, 1, NULL);
...
(1, 110201, 'Antena Central', 'Antena principal del sector', '-13.451882343831118, -76.15656155438536', 'Av. Principal 123', '2025-01-29 06:57:11', NULL, NULL, 1, NULL, NULL);
...
(1, 'Chincha', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(2, 'Pueblo Nuevo', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(3, 'Grocio Prado', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(4, 'Tambo de Mora', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(5, 'Sunampe', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL);
...
(1, 'CJ 1-16', 'caja principal de san pedro de pilpa', 14, 1, '-13.417296156139193,-76.16290082198454', '2025-01-29 07:01:03', '2025-01-29 07:03:19', NULL, 1, NULL, NULL);
...
(1, 5, NULL, 'carretera sunampe', 'por la entrada', '-13.41623244160001,-76.1552674544509', '2025-01-29 07:01:34', NULL, NULL, 1, NULL, NULL);
...
(1, 1, 44, 1, 1, NULL, 'carretera sunampe', 'por la entrada', '{\"cable\":{\"plan\":\"JUN056 PLAN DUO + 01 TV = CATV 4 TV - BASICO + 1 APL\",\"periodo\":\"2025-07-29\",\"pagoinstalacion\":30,\"potencia\":-20,\"triplexor\":{\"requerido\":\"true\",\"cargador\":\"false\"},\"conector\":{\"numeroconector\":2,\"precio\":\"1.50\"},\"splitter\":[{\"cantidad\":1,\"tipo\":\"1x5\"}],\"cable\":{\"metrosadicionales\":15,\"preciometro\":\"1.3\"},\"tipoEntrada\":{\"fila\":[\"1\"],\"puerto\":2},\"idcaja\":1,\"sintonizadores\":[]},\"costo\":{\"nap\":{\"gpon\":-12,\"catv\":-12},\"casa\":{\"gpon\":-12,\"catv\":-12},\"cablecosto\":{\"numerosintotizadores\":0,\"costoalquilersintotizador\":0,\"costocable\":19.5,\"costoconector\":\"3.00\",\"cantidadcable\":15,\"preciocable\":\"1.3\",\"precioconector\":\"1.50\",\"cantidadconector\":2,\"detalle\":\"todo operativo\"}},\"idcaja\":1}', '-13.41623244160001,-76.1552674544509', '2025-01-29', '2025-01-29', NULL, 'llamar al 907277520', '2025-01-29 07:01:35', NULL, NULL, NULL, NULL),
(2, 1, 3, 1, 1, NULL, 'Av. Paraiso', 'en el restaurante Tradiciones chinchanas', '{\"fibraoptica\":{\"usuario\":\"ALETAL2\",\"claveacceso\":\"@ALETAL2\",\"vlan\":1,\"periodo\":\"2025-07-29\",\"plan\":\"Plan Internet 110MB - 70\",\"potencia\":-20,\"router\":{\"ssid\":\"admin\",\"seguridad\":\"Gpon26\",\"codigobarra\":\"JHAB1287297\",\"ip\":\"192.168.568.182\",\"marca\":\"Micronics\",\"modelo\":\"Router Mercusys MR-70X\",\"serie\":\"ABC-123\",\"banda\":[\"5G\"],\"numeroantena\":2,\"catv\":true,\"ingresouserrouter\":\"Alex\",\"ingresopass\":\"Alex123\"},\"detalles\":\"todo operativo \",\"repetidores\":[]},\"tipoentrada\":{\"fila\":[\"1\"],\"puerto\":2},\"idcaja\":1}', '-13.419921627043415,-76.16419946744936', '2025-01-29', '2025-01-29', NULL, 'llamar al 907277520', '2025-01-29 07:03:19', NULL, NULL, NULL, NULL);
...
(1, '20123456789', 'Luis García', 'Servicios Informáticos S.A.', 'InfoTech', '987654321', 'info@infotech.com', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(2, '20234567890', 'Ana Martínez', 'Soluciones Empresariales S.R.L.', 'SolEmp', '976543210', 'contacto@solemp.com', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(3, '20345678901', 'Carlos Lopez', 'Desarrollo y Consultoría', 'DevConsult', '965432109', 'consultoria@devconsult.com', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(4, '20456789012', 'María Pérez', 'Tecnología Avanzada', 'TechAdvance', '954321098', 'info@techadvance.com', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(5, '20567890123', 'José Fernández', 'Consultores Asociados', 'ConAsociados', '943210987', 'contacto@conasociados.com', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(6, '20678901234', 'Claudia Torres', 'Innovación Digital S.A.C.', 'InnovaDigital', '932109876', 'info@innovadigital.com', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(7, '20789012345', 'Andrés Ruiz', 'Gestión de Proyectos S.R.L.', 'ProyGest', '921098765', 'contacto@proygest.com', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(8, '20890123456', 'Laura Sánchez', 'Soluciones Logísticas', 'LogiSol', '910987654', 'info@logisol.com', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(9, '20901234567', 'Ricardo Jiménez', 'Tecnologías de la Información', 'TechInfo', '909876543', 'contacto@techinfo.com', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(10, '21012345678', 'Patricia Silva', 'Estrategias Comerciales S.A.', 'EstrategiaCom', '898765432', 'info@estrategiacom.com', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(11, '21123456789', 'Javier Morales', 'Desarrollo Web S.R.L.', 'WebDev', '887654321', 'contacto@webdev.com', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(12, '21234567890', 'Verónica Castro', 'Consultoría Financiera', 'FinanConsult', '876543210', 'info@financonsult.com', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(13, '21345678901', 'Fernando Delgado', 'Servicios de Marketing', 'MarketServ', '865432109', 'contacto@marketserv.com', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(14, '21456789012', 'Mónica Herrera', 'Desarrollo de Software', 'SoftDev', '854321098', 'info@softdev.com', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(15, '21567890123', 'Santiago Castillo', 'Consultores de Negocios', 'ConNegocios', '843210987', 'contacto@connegocios.com', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(16, '21678901234', 'Elena Mendoza', 'Soluciones Ambientales', 'AmbiSol', '832109876', 'info@ambisol.com', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(17, '21789012345', 'Pablo Romero', 'Sistemas Integrales S.R.L.', 'SistInteg', '821098765', 'contacto@sistinteg.com', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(18, '21890123456', 'Gabriela López', 'Innovación Empresarial', 'InnovaEmp', '810987654', 'info@innovaemp.com', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(19, '21901234567', 'Hugo Pérez', 'Desarrollo Sostenible', 'DesaSostenible', '809876543', 'contacto@desasostenible.com', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(20, '22012345678', 'Raquel Salas', 'Tecnologías Avanzadas', 'TechAvan', '798765432', 'info@techavan.com', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(21, '22123456789', 'Diego García', 'Soluciones Creativas S.A.C.', 'CreaSol', '787654321', 'contacto@creasol.com', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(22, '22234567890', 'Susana Ríos', 'Asesoría en TI', 'AsesoriaTI', '776543210', 'info@asesoriati.com', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(23, '22345678901', 'Jorge Castro', 'Innovaciones Técnicas', 'InnovaTec', '765432109', 'contacto@innovatec.com', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(24, '22456789012', 'Carla Ramos', 'Desarrollo de Aplicaciones', 'AppDev', '754321098', 'info@appdev.com', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(25, '22567890123', 'Nicolás Mendoza', 'Gestión de Recursos S.R.L.', 'RecursosGest', '743210987', 'contacto@recursosgest.com', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(26, '22678901234', 'Marta Ortega', 'Consultoría en Procesos', 'ProcCon', '732109876', 'info@proccon.com', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(27, '22789012345', 'Mauricio López', 'Soluciones Educativas', 'EduSol', '721098765', 'contacto@edusol.com', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(28, '22890123456', 'Adriana Flores', 'Marketing Digital', 'DigitalMarket', '710987654', 'info@digitalmarket.com', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(29, '22901234567', 'Felipe Morales', 'Consultoría Jurídica', 'JuridConsult', '709876543', 'contacto@juridconsult.com', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(30, '23012345678', 'Olga Fernández', 'Estrategias de Ventas', 'EstrategiaVentas', '698765432', 'info@estrategiaventas.com', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(31, '23123456789', 'Ignacio Torres', 'Servicios de Diseño', 'DiseñoServ', '687654321', 'contacto@disenoserv.com', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(32, '23234567890', 'Natalia Guzmán', 'Innovación en Salud', 'SaludInnova', '676543210', 'info@saludinnova.com', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(33, '23345678901', 'Ricardo Salcedo', 'Soluciones de Recursos Humanos', 'RecursosHum', '665432109', 'contacto@recursoshum.com', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(34, '23456789012', 'Silvia Morales', 'Consultoría en Tecnologías', 'TecConsult', '654321098', 'info@tecconsult.com', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(35, '23567890123', 'Raúl Gutiérrez', 'Servicios de Ingeniería', 'EngiServ', '643210987', 'contacto@engiserv.com', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(36, '23678901234', 'Camila Sánchez', 'Innovación Empresarial S.R.L.', 'EmpInnova', '632109876', 'info@empinnova.com', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(37, '23789012345', 'Julio Ramírez', 'Gestión Ambiental S.A.C.', 'AmbientalGest', '621098765', 'contacto@ambientalgest.com', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(38, '23890123456', 'Sofía Herrera', 'Tecnologías Sustentables', 'SustenTech', '610987654', 'info@sustentech.com', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(39, '23901234567', 'David Torres', 'Asesoría Financiera S.A.', 'FinanAses', '609876543', 'contacto@finanases.com', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(40, '24012345678', 'María Vargas', 'Consultores Legales', 'LegalConsult', '598765432', 'info@legalconsult.com', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(41, '24123456789', 'Esteban Gálvez', 'Desarrollo de Proyectos', 'ProyectDev', '587654321', 'contacto@proyectdev.com', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(42, '24234567890', 'Daniela Ortiz', 'Servicios Médicos S.A.C.', 'MedServ', '576543210', 'info@medserv.com', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(43, '24345678901', 'Gustavo Peña', 'Consultores en Ventas', 'VentasConsult', '565432109', 'contacto@ventasconsult.com', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL);
...
(1, 'TP Link', NULL, NULL, '2025-01-29 06:57:08', NULL, 1, NULL),
(2, 'Phyhome', NULL, NULL, '2025-01-29 06:57:08', NULL, 1, NULL),
(3, 'VSOL', NULL, NULL, '2025-01-29 06:57:08', NULL, 1, NULL),
(4, 'Ubiquiti', NULL, NULL, '2025-01-29 06:57:08', NULL, 1, NULL),
(5, 'Huawei', NULL, NULL, '2025-01-29 06:57:08', NULL, 1, NULL),
(6, 'Micronics', NULL, NULL, '2025-01-29 06:57:08', NULL, 1, NULL),
(7, 'Mercusys', NULL, NULL, '2025-01-29 06:57:08', NULL, 1, NULL),
(8, 'Generico', NULL, NULL, '2025-01-29 06:57:08', NULL, 1, NULL),
(9, 'Sin Marca', NULL, NULL, '2025-01-29 06:57:08', NULL, 1, NULL);
...
(1, 'MF 1', 'primera mufa', '-13.414464196533709,-76.14717459034142', 'Panamericana', '2025-01-29 06:59:26', NULL, NULL, 1, NULL, NULL);
...
(1, '{\"id_servicio\": [3]}', 'Plan Internet Corporativo 300MB - 270', 65.00, '{\"bajada\":{\"maxima\":300,\"minima_garantizada\":270},\"subida\":{\"maxima\":300,\"minima_garantizada\":270}}', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(2, '{\"id_servicio\": [3]}', 'Plan Internet 55MB - 50', 60.00, '{\"bajada\":{\"maxima\":55,\"minima_garantizada\":50},\"subida\":{\"maxima\":55,\"minima_garantizada\":50}}', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(3, '{\"id_servicio\": [3]}', 'Plan Internet 110MB - 70', 70.00, '{\"bajada\":{\"maxima\":110,\"minima_garantizada\":70},\"subida\":{\"maxima\":110,\"minima_garantizada\":70}}', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(4, '{\"id_servicio\": [3]}', 'Plan Internet 155MB - 90', 68.00, '{\"bajada\":{\"maxima\":155,\"minima_garantizada\":90},\"subida\":{\"maxima\":155,\"minima_garantizada\":90}}', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(5, '{\"id_servicio\": [3]}', 'Plan Internet 165MB - 95', 67.00, '{\"bajada\":{\"maxima\":165,\"minima_garantizada\":95},\"subida\":{\"maxima\":165,\"minima_garantizada\":95}}', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(6, '{\"id_servicio\": [3]}', 'Plan Internet MIGRA 210MB - 110', 69.00, '{\"bajada\":{\"maxima\":210,\"minima_garantizada\":110},\"subida\":{\"maxima\":210,\"minima_garantizada\":110}}', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(7, '{\"id_servicio\": [3]}', 'Plan Internet 280MB - 150', 66.00, '{\"bajada\":{\"maxima\":280,\"minima_garantizada\":150},\"subida\":{\"maxima\":280,\"minima_garantizada\":150}}', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(8, '{\"id_servicio\": [1,3]}', 'Plan Internet Duo Bronce 110MB - 80', 100.00, '{\"bajada\":{\"maxima\":110,\"minima_garantizada\":80},\"subida\":{\"maxima\":110,\"minima_garantizada\":80}}', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(9, '{\"id_servicio\": [1,3]}', 'Plan Internet Duo Plata', 105.00, '{\"bajada\":{\"maxima\":100,\"minima_garantizada\":80},\"subida\":{\"maxima\":100,\"minima_garantizada\":80}}', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(10, '{\"id_servicio\": [1,3]}', 'Plan Internet Duo Oro 210MB - 120', 110.00, '{\"bajada\":{\"maxima\":210,\"minima_garantizada\":120},\"subida\":{\"maxima\":210,\"minima_garantizada\":120}}', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(11, '{\"id_servicio\": [1,3]}', 'Plan Internet Duo 180MB - 100', 95.00, '{\"bajada\":{\"maxima\":180,\"minima_garantizada\":100},\"subida\":{\"maxima\":180,\"minima_garantizada\":100}}', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(12, '{\"id_servicio\": [1,3]}', 'Plan Internet Duo 410MB - 140', 120.00, '{\"bajada\":{\"maxima\":410,\"minima_garantizada\":140},\"subida\":{\"maxima\":410,\"minima_garantizada\":140}}', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(13, '{\"id_servicio\": [1,3]}', 'Plan Internet Full Duo 2 - 190MB - 99', 115.00, '{\"bajada\":{\"maxima\":190,\"minima_garantizada\":99},\"subida\":{\"maxima\":190,\"minima_garantizada\":99}}', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(14, '{\"id_servicio\": [1,3]}', 'Plan Internet Full Duo 3 - 260MB - 120', 130.00, '{\"bajada\":{\"maxima\":260,\"minima_garantizada\":120},\"subida\":{\"maxima\":260,\"minima_garantizada\":120}}', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(15, '{\"id_servicio\": [1,3]}', 'Plan Internet Full Duo 4 - 320MB - 135', 125.00, '{\"bajada\":{\"maxima\":320,\"minima_garantizada\":135},\"subida\":{\"maxima\":320,\"minima_garantizada\":135}}', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(16, '{\"id_servicio\": [1,3]}', 'Plan Internet Full Duo 5 - 410MB - 159', 140.00, '{\"bajada\":{\"maxima\":410,\"minima_garantizada\":159},\"subida\":{\"maxima\":410,\"minima_garantizada\":159}}', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(17, '{\"id_servicio\": [2]}', 'Plan TamboMora Residencial 40MB - 50', 120.00, '{\"bajada\":20,\"subida\":20}', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(18, '{\"id_servicio\": [2]}', 'Techo Videna Residencial 20MB - 50', 110.00, '{\"bajada\":20,\"subida\":20}', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(19, '{\"id_servicio\": [2]}', 'Plan Salvador Residencial 20MB - 50', 115.00, '{\"bajada\":20,\"subida\":20}', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(20, '{\"id_servicio\": [2]}', 'Plan Sunampe Residencial 15MB', 105.00, '{\"bajada\":20,\"subida\":20}', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(21, '{\"id_servicio\": [2]}', 'Plan Chacarita Residencial 40MB - 50', 130.00, '{\"bajada\":20,\"subida\":20}', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(22, '{\"id_servicio\": [2]}', 'Plan Chacarita Residencial 40MB - 80', 140.00, '{\"bajada\":20,\"subida\":20}', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(23, '{\"id_servicio\": [2]}', 'Plan Satelite Residencial 40MB - 70', 125.00, '{\"bajada\":20,\"subida\":20}', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(24, '{\"id_servicio\": [2]}', 'Plan TamboMora Residencial 40MB', 120.00, '{\"bajada\":20,\"subida\":20}', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(25, '{\"id_servicio\": [2]}', 'Plan Sunampe Corporativo 50MB - 210', 150.00, '{\"bajada\":20,\"subida\":20}', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(26, '{\"id_servicio\": [2]}', 'Plan Sunampe Corporativo 50MB - 280', 160.00, '{\"bajada\":20,\"subida\":20}', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(27, '{\"id_servicio\": [2]}', 'Plan Chincha Corporativo 50MB - 280', 170.00, '{\"bajada\":20,\"subida\":20}', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(28, '{\"id_servicio\": [2]}', 'Plan Grocio Residencial 10MB', 90.00, '{\"bajada\":20,\"subida\":20}', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(29, '{\"id_servicio\": [2]}', 'Plan Grocio Residencial 12MB', 95.00, '{\"bajada\":20,\"subida\":20}', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(30, '{\"id_servicio\": [2]}', 'Plan Grocio Residencial 20MB', 100.00, '{\"bajada\":20,\"subida\":20}', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(31, '{\"id_servicio\": [2]}', 'Plan TechoPropio Residencial 10MB', 85.00, '{\"bajada\":20,\"subida\":20}', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(32, '{\"id_servicio\": [2]}', 'Plan TechoPropio Residencial 20MB', 105.00, '{\"bajada\":20,\"subida\":20}', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(33, '{\"id_servicio\": [2]}', 'Plan Satelite Residencial 10MB', 80.00, '{\"bajada\":20,\"subida\":20}', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(34, '{\"id_servicio\": [2]}', 'Plan Satelite Residencial 12MB', 90.00, '{\"bajada\":20,\"subida\":20}', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(35, '{\"id_servicio\": [2]}', 'Plan Satelite Residencial 20MB', 110.00, '{\"bajada\":20,\"subida\":20}', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(36, '{\"id_servicio\": [2]}', 'Plan CruzBlanca Residencial 10MB', 75.00, '{\"bajada\":20,\"subida\":20}', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(37, '{\"id_servicio\": [2]}', 'Plan CruzBlanca Residencial 12MB', 85.00, '{\"bajada\":20,\"subida\":20}', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(38, '{\"id_servicio\": [2]}', 'Plan CruzBlanca Residencial 20MB', 95.00, '{\"bajada\":20,\"subida\":20}', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(39, '{\"id_servicio\": [2]}', 'Plan Salvador Residencial 10MB', 70.00, '{\"bajada\":20,\"subida\":20}', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(40, '{\"id_servicio\": [2]}', 'Plan Salvador Residencial 20MB', 100.00, '{\"bajada\":20,\"subida\":20}', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(41, '{\"id_servicio\": [2]}', 'Plan Salvador Residencial 10MB', 75.00, '{\"bajada\":20,\"subida\":20}', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(42, '{\"id_servicio\": [1]}', 'ABRO1 SERVICIO CATV 2 TV-BASICO + 1 APL - DUO 80 ANTERIOR', 45.00, NULL, '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(43, '{\"id_servicio\": [1]}', 'AGOS080 NUEVO SERVICIO SOLO CATV3 TV - BASICO + 1 APL', 50.00, NULL, '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(44, '{\"id_servicio\": [1]}', 'JUN056 PLAN DUO + 01 TV = CATV 4 TV - BASICO + 1 APL', 48.00, NULL, '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(45, '{\"id_servicio\": [1]}', 'MAY053 NUEVO SERVICIO CATV 3 TV - BASICO + 1 APL', 49.00, NULL, '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(46, '{\"id_servicio\": [1]}', 'MAY054 NUEVO SERVICIO SOLO CATV 3 TV - BASICO + 1 APL', 47.00, NULL, '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(47, '{\"id_servicio\": [1]}', 'MAY055 NUEVO SERVICIO DUO CATV 4 TV - BASICO + 1 APL', 46.00, NULL, '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL);
...
(1, 'DNI', '12345678', 'PEREZ GARCIA', 'JUAN', '987654321', 'Peruano', 'juan.perezgarcia@example.com', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(2, 'DNI', '73884605', 'MATTA RAMOS', 'JESUS EDUARDO', '904217929', 'Peruano', NULL, '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(3, 'DNI', '73310144', 'MESIAS TASAYCO', 'BRAYAN', '933293445', 'Peruano', NULL, '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(4, 'DNI', '72845296', 'DE LA CRUZ PEÑALOZA', 'ELOY ALEXANDER', '920520306', 'Peruano', NULL, '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(5, 'DNI', '71592495', 'TALLA SARAVIA', 'ALEXIS ALEXANDER', '907277520', 'Peruano', NULL, '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(6, 'DNI', '41821854', 'DE LA CRUZ NAPA', 'JOSE IVAN', '945780584', 'PERUANO', NULL, '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL);
...
(1, 1, 1, 1, 'Router TP-Link TL-WR940N', 120.00, '12879878974564654654', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(2, 2, 2, 2, 'Sintonizador Phyhome PH-100', 80.00, 'AKJSBKJCKJ161', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(3, 3, 3, 1, 'Repetidor VSOL VS-300', 150.00, 'AKJBAJBUIB1898', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(4, 4, 4, 2, 'Consumibles Huawei HW-500', 50.00, 'ABC-abc-1234', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(5, 5, 5, 1, 'Herramientas Micronics MC-200', 200.00, 'GAAK1129893J', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(6, 6, 1, 2, 'Router Mercusys MR-70X', 130.00, 'JHAB1287297', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(7, 7, 2, 1, 'Sintonizador Generico GN-100', 70.00, 'AJSFKA128763', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(8, 8, 3, 2, 'Repetidor Sin Marca SM-300', 90.00, '69KAJSBKJB12', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(9, 9, 4, 1, 'Consumibles TP-Link TL-500', 60.00, 'KJBKAJBKAJBC', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(10, 9, 5, 2, 'Herramientas Phyhome PH-200', 110.00, 'KJASBFKJA9', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL);
...
(1, 1, 1, '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(2, 2, 2, '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(3, 3, 3, '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(4, 4, 4, '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(5, 5, 5, '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL);
...
(1, 110210, 'San Pedro de Pilpa', 'para toda la zona', NULL, '-13.416878715701749,-76.16511096221281', '2025-01-29 07:00:17', NULL, NULL, 1, NULL, NULL);
...
(1, 'CABL', 'Cable', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(2, 'WISP', 'Wireless Internet Service Provider', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(3, 'FIBR', 'Fibra Óptica', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL);
...
(1, 1, 'Chincha', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(2, 1, 'Rosedal', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(3, 1, 'Señor de los Milagros', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(4, 1, 'Cruz Blanca', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(5, 1, 'Condorillo', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(6, 2, 'Salvador', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(7, 2, 'Satélite', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(8, 3, 'Grocio', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(9, 4, 'Tambo de Mora', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(10, 4, 'Tambo Plaza Antiguo', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(11, 5, 'Sunampe', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(12, 5, 'San Ignacio', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(13, 5, 'San Pilpa', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(14, 5, 'Chacarita', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL);
...
(1, 'Router', '2025-01-29 06:57:08', NULL, NULL, NULL, 1, NULL),
(2, 'Sintonizador', '2025-01-29 06:57:08', NULL, NULL, NULL, 1, NULL),
(3, 'Repetidor', '2025-01-29 06:57:08', NULL, NULL, NULL, 1, NULL),
(4, 'Consumibles', '2025-01-29 06:57:08', NULL, NULL, NULL, 1, NULL),
(5, 'Herramientas', '2025-01-29 06:57:08', NULL, NULL, NULL, 1, NULL);
...
(1, 'Unidades', '2025-01-29 06:57:08', NULL, 1, NULL),
(2, 'Metros', '2025-01-29 06:57:08', NULL, 1, NULL);
...
(1, 1, 'Lgarcia', '$2y$10$v33I.gsFvcSz5fGO9zCD6OeTmL65ivYUP63ZlZ7xcQrv100eVaRRK', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(2, 2, 'Mramos', '$2y$10$v33I.gsFvcSz5fGO9zCD6OeTmL65ivYUP63ZlZ7xcQrv100eVaRRK', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(3, 3, 'Mtasayco', '$2y$10$v33I.gsFvcSz5fGO9zCD6OeTmL65ivYUP63ZlZ7xcQrv100eVaRRK', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(4, 4, 'Dpeñaloza', '$2y$10$v33I.gsFvcSz5fGO9zCD6OeTmL65ivYUP63ZlZ7xcQrv100eVaRRK', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(5, 5, 'Tsaravia', '$2y$10$v33I.gsFvcSz5fGO9zCD6OeTmL65ivYUP63ZlZ7xcQrv100eVaRRK', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL),
(6, 6, 'JoseIvan', '$2y$10$jK9ALyQ6CabQCH1U3PqajeqCJQrVflPVEAl0GIU1OLJpp9GsKXCEG', '2025-01-29 06:57:08', NULL, NULL, 1, NULL, NULL);
...
-- (Véase abajo para la vista actual)
--
CREATE TABLE `vw_averias_contar_ficha_vacia` (
...
-- (Véase abajo para la vista actual)
--
CREATE TABLE `vw_averias_listar_ficha_null` (
`id_soporte` int(11)
,`id_contrato` int(11)
...
-- (Véase abajo para la vista actual)
--
CREATE TABLE `vw_clientes_contar_con_ficha_llena` (
...
-- (Véase abajo para la vista actual)
--
CREATE TABLE `vw_contratos_contar_ficha_vacia` (
...
-- (Véase abajo para la vista actual)
--
CREATE TABLE `vw_contratos_listar_ficha_null` (
`id_contrato` int(11)
,`nombre_cliente` varchar(100)
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vw_averias_contar_ficha_vacia`  AS SELECT count(0) AS `total_averias_ficha_vacia` FROM `tb_soporte` WHERE `tb_soporte`.`estaCompleto` = 0 AND `tb_soporte`.`inactive_at` is null ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vw_clientes_contar_con_ficha_llena`  AS SELECT count(distinct `c`.`id_cliente`) AS `total_clientes_con_ficha_llena` FROM `tb_contratos` AS `c` WHERE `c`.`ficha_instalacion` is not null AND `c`.`inactive_at` is null ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vw_clientes_obtener`  AS SELECT `c`.`id_cliente` AS `id_cliente`, coalesce(concat(`p`.`nombres`,', ',`p`.`apellidos`),`e`.`razon_social`) AS `nombre_cliente`, coalesce(`p`.`nro_doc`,`e`.`ruc`) AS `codigo_cliente`, coalesce(`p`.`email`,`e`.`email`) AS `email_cliente`, coalesce(`p`.`telefono`,`e`.`telefono`) AS `telefono_cliente`, `c`.`direccion` AS `direccion_cliente`, `c`.`referencia` AS `referencia_cliente`, `c`.`coordenadas` AS `coordenadas_cliente` FROM (((`tb_clientes` `c` left join `tb_personas` `p` on(`c`.`id_persona` = `p`.`id_persona` and `p`.`inactive_at` is null)) left join `tb_empresas` `e` on(`c`.`id_empresa` = `e`.`id_empresa` and `e`.`inactive_at` is null)) left join `tb_contratos` `ct` on(`c`.`id_cliente` = `ct`.`id_cliente` and `ct`.`inactive_at` is null)) WHERE `c`.`inactive_at` is null AND `ct`.`ficha_instalacion` is not null GROUP BY `c`.`id_cliente`, coalesce(concat(`p`.`nombres`,', ',`p`.`apellidos`),`e`.`razon_social`), coalesce(`p`.`nro_doc`,`e`.`ruc`), coalesce(`p`.`email`,`e`.`email`), coalesce(`p`.`telefono`,`e`.`telefono`), `c`.`direccion`, `c`.`referencia`, `c`.`coordenadas` ;
...
--
-- Indices de la tabla `tb_antenas`
--
ALTER TABLE `tb_antenas`
  ADD PRIMARY KEY (`id_antena`),
  ADD UNIQUE KEY `nombre` (`nombre`),
  ADD UNIQUE KEY `anten_uk_antena` (`nombre`),
  ADD KEY `fk_sector_antena` (`id_distrito`);
...
--
-- AUTO_INCREMENT de la tabla `tb_antenas`
--
ALTER TABLE `tb_antenas`
  MODIFY `id_antena` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=2;
...
--
ALTER TABLE `tb_contactabilidad`
  MODIFY `id_contactabilidad` int(11) NOT NULL AUTO_INCREMENT;
...
-- Eventos
--
CREATE DEFINER=`root`@`localhost` EVENT `ev_inhabilitar_contactos` ON SCHEDULE EVERY 1 DAY STARTS '2025-01-29 06:57:17' ON COMPLETION NOT PRESERVE ENABLE DO BEGIN
